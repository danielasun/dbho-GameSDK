...
struct C##name##Creator : public IGameObjectExtensionCreatorBase\
		...
		IGameObjectExtensionPtr Create()\
			{\
			return gEnv->pEntitySystem->CreateComponent<C##name>();\
			}\
...
struct C##name##Creator : public IGameObjectExtensionCreatorBase\
		...
		IGameObjectExtensionPtr Create()\
			{\
			return gEnv->pEntitySystem->CreateComponent<C##impl>();\
			}\
			void GetGameObjectExtensionRMIData( void ** ppRMI, size_t * nCount )\
			{\
...
#define REGISTER_VEHICLEOBJECT(name, obj) \
	...
	// TODO: All the factory nodes that are new style should be initialized by this.
	// Worth and or necessary to get to this for all components?
